        ...
        <div class="flex flex-row h-screen bg-white rounded-lg sm:h-main" x-data="{ sidebar_extended: false, showSideBar: false }" x-cloak>
            ...
            <div class="w-full pb-0 sm:pb-10 sm:w-8/12 sm:basis-8/12 sm:grow">
                <div class="flex flex-col items-center justify-between p-5 border-b sm:h-20 sm:flex-row">
                    ...
                    <div @class([
                        ' sm:hidden flex flex-row absolute z-20 mt-24',
                        'right-0' => ar(),
                        'left-0' => en(),
                    ]) x-show="showSideBar">
                        <nav @class([
                            'bg-white shadow-lg h-screen border-white ',
                            ' pl-10 sm:pl-10' => ar(),
                            'pr-10 sm:pr-10' => en(),
                        ])>
                        <x-mobile-sidebar />
                    ...
                    <div class="flex flex-row items-center">
                        <div class="flex flex-row">
                            <button @click="showSideBar=!showSideBar" type="button"
                                class=" inline-flex items-center px-4 py-2 text-lg text-gray-400 bg-transparent rounded-lg sm:hidden hover:bg-gray-200 hover:text-gray-900 ">
                        ...
                        <div x-data="{ dropdown: false }" class=" ml-10 relative mr-10 ">
                            <div>
                                <button @click.stop="dropdown = !dropdown" type="button"
                                    ...
                                    <img class="w-8 h-8 rounded-full" src="{{ auth()->user()->profile_photo }}"
                                        alt="profile image">
                                </button>
                            </div>
                            <div @click.outside="dropdown = false" @click.stop="" x-show="dropdown"
                                class="absolute lg:right-0 left-1  w-48 mt-2 origin-top-right bg-white rounded-lg ring-1 ring-opacity-5 focus:outline-none "
                                role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button"
                                tabindex="-1">
                                <!-- Active: "bg-gray-100", Not Active: "" -->
                                @if (!auth()->user()->is_admin)
                                    <a href="{{ route('employees.profile', ['id' => auth()->user()->id]) }}"
                                        class="block px-4 py-2 text-sm text-gray-700 rounded-lg hover:bg-gray-200"
                                        role="menuitem" tabindex="-1" id="user-menu-item-0">{{ __('ui.profile') }}</a>
                                @else
                                    <a href="{{ route('home') }}"
                                        class="block px-4 py-2 text-sm text-gray-700 rounded-lg hover:bg-gray-200"
                                        role="menuitem" tabindex="-1" id="user-menu-item-0">{{ __('ui.profile') }}</a>
                                ...
                                <form method="POST" action="{{ route('logout') }}" x-data>
                                    ...
                                    <a href="{{ route('logout') }}" @click.prevent="$root.submit();"
                                        class="block px-4 py-2 text-sm text-gray-700 rounded-lg hover:bg-gray-200"
                                        role="menuitem" tabindex="-1" id="user-menu-item-2">{{ __('ui.logout') }}</a>
                                </form>
                            </div>
                        </div>
