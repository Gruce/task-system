<div x-show="selected == 0" class="flex flex-col gap-6">
    <div x-data="{ title: false, project: false }" class="flex items-center justify-between pb-2 border-b-2 border-secondary-50">
        ...
        <div @click.outside="project = false" class="flex items-center gap-4 cursor-pointer group">
            ...
            <select wire:model="task.project_id" wire:keydown.enter="edit_name"  x-show="project" class="block w-full px-4 py-1 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500">
    ...
    <div @click.outside="description = false" x-data="{description: false}">
        ...
        <textarea wire:model="task.description" wire:keydown.enter="edit_name"  x-show="description" id="message" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded border border-gray-300 focus:ring-primary-500 focus:border-primary-500" placeholder="Description"></textarea>
    </div>
</div>
